...
import { ChevronDown, User, Users, Calendar, LogOut, Settings, Plus, Search, Home } from 'lucide-react';
...
const UnifiedNavigation = () => {
  ...
  return (
    <div className="flex items-center space-x-2">
      ...
      <Button variant="ghost" onClick={() => navigate("/")} className="hidden sm:flex items-center space-x-1">
        <Home className="h-4 w-4" />
        <span>Home</span>
      </Button>

      <Button variant="ghost" onClick={() => navigate("/search")} className="hidden sm:flex items-center space-x-1">
        <Search className="h-4 w-4" />
        <span>Search</span>
      </Button>

      {canAccessTeams && (
        <Button variant="ghost" onClick={() => navigate("/teams")} className="hidden sm:flex items-center space-x-1">
      ...
      {(hasRole('organiser') || hasRole('tournament_organizer')) && (
        <Button variant="ghost" onClick={() => navigate("/organiser/tournament")} className="hidden sm:flex items-center space-x-1">
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" className="flex items-center space-x-2">
            ...
            <span className="hidden md:inline">{profile?.display_name || user.email?.split('@')[0] || 'User'}</span>
